...
func (r *CronJobReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// step 1: Load the Cronjob by name
	...
	// step 2: List all active jobs, and update the status
	...
	// setp 3: clean up old jobs according to the history limit
	...
	// setp 4: check if we're suspended
	if cronJob.Spec.Suspend != nil && *cronJob.Spec.Suspend {
		log.V(1).Info("cronjob suspended, skipping")
		return ctrl.Result{}, nil
	}

	// setp 5: get the next scheduled run
	...
	//step 6: Run a new job if it's on schedule , not past the dealine , and not blocked by our concurrency policy.
	if missedRun.IsZero() {
	...
	// update state condition to reflect missed deadline.
	meta.SetStatusCondition(&cronJob.Status.Conditions, metav1.Condition{
		Type:    typeDegradedCronJob,
		Status:  metav1.ConditionTrue,
	...
	// setp 7:  Requeue when we either see a running job or it’s time for the next scheduled run
